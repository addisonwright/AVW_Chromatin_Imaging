...
def fit_seed_points_base(im,centers,width_zxy=[1.,1.,1.],radius_fit=5,n_max_iter = 10,max_dist_th=0.25):
    ...
    if len(x)>0:
        ...
        return np.array([])
def get_ims_meds(files,num_cols=3,median_fl=None,overwrite=False,tag='mid',verbose=False):
    """Given a list of dax filenames 
    ...
    if compute:
        ...
        for file_ in files:
            if verbose:
                print file_
            ims_ = io.dax_im(file_,num_col=num_cols).get_mids(tag=tag)
...
def linrescale(im_mean1,im_mean2,n=100000):
    ...
    return (im_mean2-c)/m
def return_candidate_centers(im,dist_th=20,ex=20,ey=20,ez=2,gmin=2,gmax=5,filtsz=5,ths=0, th_seed=4,plt_val=False,save_file=None):
    zs,xs,ys = im.shape
    centersh = get_seed_points_base(im,gfilt_size_min=gmin, gfilt_size_max=gmax, filt_size=filtsz, th_std=ths, th_seed=th_seed, return_h=True)
...
def get_STD_beaddrift_v2(ims_beads,coord_sel=None,sz_ex=50,desired_nbeads = 20, 
                      ...
                      ref=None,force=False,save=True,save_file='temp.pkl',ind=0):
    ...
    if repeat:
        ...
        for iim in bad_inds+range(len(txyzs_both),len(ims_beads)):
            print "Aligning "+str(ind)+'.'+str(iim+1)
            im = ims_beads[iim]
