...
def slice_file(fl,sx,sy,sz,minx=0,maxx=np.inf,miny=0,maxy=np.inf,minz=0,maxz=np.inf,stridex=1,stridey=1,stridez=1):
    ...
    return data
class dax_im():
    def __init__(self,dax_fl,num_col=3,bead_col=None,color=0):
        #internalize
        self.color = color
        self.dax_fl = dax_fl
        ...
        self.shape = (int((end-start)/self.num_col),self.image_height,self.image_width)
    ...
    def get_mids(self,tag='mid',minx=0,maxx=np.inf,miny=0,maxy=np.inf):
        ...
        for ind_col in range(num_col):
            ...
            if tag=='all':
                im_block = self.get_slice(minx=self.start(ind_col),maxx=self.end(ind_col),miny=miny,maxy=maxy,minz=minx,maxz=maxx,
...
def get_ims_fov_daxmap(files_,col_tags = ['beads','647','750']):
    """Given folders and a fov, this returns ims_signal,ims_beads,names"""
    daxs_signal,names_signal,daxs_beads,names_beads=[],[],[],[]
    
    for dax_fl in files_:
        tag=os.path.basename(dax_fl)
        if os.path.exists(dax_fl):
            num_cols = len(col_tags)
            for icol,col_tag in enumerate(col_tags):
                if col_tag=='beads':
                    names_beads+= [tag+'_'+col_tag]
                    dax_obj =dax_im(dax_fl, num_col=num_cols, bead_col=0,color=icol)
                    daxs_beads.append(dax_obj)
                elif col_tag == 'discard':
                    pass
                else:
                    dax_obj =dax_im(dax_fl, num_col=num_cols, bead_col=0,color=icol)
